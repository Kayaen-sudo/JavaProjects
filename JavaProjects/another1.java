...
public class another1 {
	...
	Random rand  = new Random();
		...
		while(p1 < Win && p2 < Win){
			...
			if(p1 == 0 ||  p2 ==0){
			dice1 = rand.nextInt(2) + 1; //Rolls the first dice
			dice2 = rand.nextInt(2) + 1; //Rolls the second dice
			...
			System.out.println("Player 1 current score is: " +p1);//Displays player one's score
					 ...
					 if(dice1 == dice2){
							dice1 = rand.nextInt(2) + 1; //Rolls the first dice again
							dice2 = rand.nextInt(2) + 1; //Rolls the second dice again
							p1 += dice1 + dice2;		//Cummulates player one vice scores and adds them
							System.out.println("Player 1 rolls again");//Displays player one's scores
